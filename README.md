...
Build the docker image with:
        ...
        docker build -t alegator-backend .
...
port with `docker ps`:
        ...
        docker run --env-file .env --P alegator-backend
        docker ps
...
To run a custom command using the image (for example, db migrations):
        ...
        docker run --env-file .env alegator-backend python manage.py migrate

To run a Django shell inside the container:

        docker run --env-file .env -t alegator-backend
